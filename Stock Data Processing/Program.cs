                                    ...
                                    foreach (StockAccount.Records i in stockAccount.Company_Records)
                                    {
                                        if (i.Symbol.Equals(shareName))
                                        {
                                            ...
                                            if (stockNo > 0 && stockNo < record.StockDetails[shareName])
                                            {
                                                ...
                                                File.WriteAllText(@"F:\BL_.net278\Demo\OOps\OOPs\Stock Data Processing\CustomerRecords.json", JsonConvert.SerializeObject(customer));
                                                File.WriteAllText(@"F:\BL_.net278\Demo\OOps\OOPs\Stock Data Processing\StockAccount.json", JsonConvert.SerializeObject(stockAccount));
                                                File.WriteAllText(@"F:\BL_.net278\Demo\OOps\OOPs\Stock Data Processing\Transaction.json", JsonConvert.SerializeObject(transaction));
